...
class PoeTradeScraper:
	...
	def __init__(self, league):
		...
		self.league = league
		self.variants = ['Softcore', 'Hardcore']
		...
		self.table_info=["(time, exchange, have_name, have_value, want_name, want_value, ratio_h_w, ratio_w_h)", "(%s, %s, %s, %s, %s, %s, %s, %s)"]
	...
	def scrape(self):
		...
		for variant in self.variants:
			self.log.write('\n\nGetting '+variant+' exchanges.')
			self.log.write('\n------------------------------------------------------')
			print('Getting '+variant+' exchanges.')
			...
			for want in self.currencies:
				...
				for have in self.currencies:
					...
					# Get the HTML Source for Softcore or Hardcore
					if variant == 'Softcore':
						url = 'http://currency.poe.trade/search?league='+self.league+'&online=x&want=' + str(want) + '&have=' + str(have)
					else:
						url = 'http://currency.poe.trade/search?league='+self.variants[1]+'+'+self.league+'&online=x&want=' + str(want) + '&have=' + str(have)
					...
					self.db.insert('poe_currency_'+self.league+'_'+variant,self.table_info,self.data)
